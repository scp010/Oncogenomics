...
specimenGeneral=select(clean_20190620_DI_v2,studyID,Spec.Type)%>%
  group_by(studyID,Spec.Type)%>%
    tally()%>%
      dcast(studyID~Spec.Type,value.var="n")
  #Build Surgeon and studyID table, remove blanks in Surgeon
testSurgeon=select(clean_20190620_DI_v2,studyID,Surgeon)%>%
  unique()%>%
    na_if("")%>%
      na.omit
  ...
  unique()%>%
    group_by(studyID,Disease.History)%>%
      arrange(Spec.Collection.Date)%>%
        slice(1L)
  #Join Date and Surgeon tables
dateSurgeon=left_join(date,testSurgeon,by="studyID")
  #Join Specimen table with date/Surgeon table
specimenGeneral=left_join(specimenGeneral,dateSurgeon,by="studyID")
  #Reorganize
specimenGeneral=specimenGeneral[,c(1,9,2:ncol(specimenGeneral))]%>%
